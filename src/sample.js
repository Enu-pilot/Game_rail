...
export function sampleDoc() {
  ...
  // 出区線と入出区線を結ぶシーサスクロッシング（両渡り線）。
  // 2本の線路にまたがる装置なので、渡り線2本＝転てつ器4・ダイヤモンド1で構成する
  t.push(T('入出区渡り1番', 'other', [{ x: 556, y: 80 }, { x: 596, y: 110 }], { note: 'シーサスの渡り線' }));
  t.push(T('入出区渡り2番', 'other', [{ x: 596, y: 80 }, { x: 556, y: 110 }], { note: 'シーサスの渡り線' }));
